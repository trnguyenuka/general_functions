...
for (sample.id in unique(s.obj$name)){
  rawVDJ[[sample.id]] <- subset(s.obj@meta.data, s.obj@meta.data$name == sample.id) %>%
    subset(select = c(CTstrict)) %>%
    subset(is.na(CTstrict) == FALSE)
  countVDJ[[sample.id]] <- rawVDJ[[sample.id]] %>% group_by(CTstrict) %>% summarise(count = n()) %>% arrange(desc(count))
}
